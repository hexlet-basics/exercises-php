Выражение `match` из прошлого урока появилось в PHP только в восьмой версии. До этого ту же задачу решала конструкция **switch** — она встречается во многих языках и в огромном количестве существующего PHP-кода, поэтому ее важно уметь читать.
...
Напоследок сравним *switch* с `match` из прошлого урока:

* `match` — выражение и вычисляется в значение, *switch* — инструкция: чтобы получить результат, нужна переменная или `return` внутри `case`.
* `match` сравнивает строго (`===`), *switch* использует нестрогое сравнение `==`, при котором происходит автоматическое преобразование типов.
* В `match` выполняется ровно одна ветка, в *switch* без `break` происходит «проваливание» в следующий `case`.

В новом коде предпочитайте `match`, а *switch* пригодится при чтении и поддержке существующих программ.
