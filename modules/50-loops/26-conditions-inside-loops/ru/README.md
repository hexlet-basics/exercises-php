Тело цикла, как и тело функции — это место выполнения инструкций. Значит, мы можем использовать внутри него всё изученное ранее, например — условные конструкции. Так программа повторяет одно действие несколько раз, но на каждом повторе принимает решение.
...
Условия внутри циклов отлично сочетаются с агрегацией. Представьте себе функцию, которая считает, сколько раз буква входит в предложение. Пример ее работы:
...
Переменная `$count` увеличивается только в том случае, когда текущий рассматриваемый символ совпадает с ожидаемым. В остальном — это типичная агрегатная функция, которая возвращает количество нужных символов вызывающему коду.
