...
Понять такой код невозможно, поэтому лучше не использовать такие конструкции. Сам по себе инкремент в PHP обычное дело, и на отдельной строчке он читается нормально. Плохо становится тогда, когда он стоит внутри выражения и меняет переменную по ходу вычисления.
