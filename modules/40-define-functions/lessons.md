...
## Передача нескольких аргументов
  ...
  В PHP, как и в большинстве языков, при вызове функции нужно обязательно передать столько же аргументов, сколько было указано при её определении. Иначе программа выдаст ошибку `Uncaught ArgumentCountError`.
...
theory: |
  ...
  Возврат задаётся специальной инструкцией `return`. Справа от `return` помещается выражение. Любое выражение. То есть, мы можем делать вычисления сразу после `return` без создания переменной `$result`:
